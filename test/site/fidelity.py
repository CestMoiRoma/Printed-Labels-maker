...
It proved the rewrite pixel-identical to the first export (20 states) before the site's own fixes, which
change the rendering on purpose: a one-off proof, not a gate. Run it against a new Claude Design export to
see what the design changed.
"""
...
TARGETS = {
    "export": None,  # set from the command line
...
def scenario(page):
    ...
    # wait for the "home" results first: in the export, a second query sent early races with the first
...
def main(export_html, out_dir):
    TARGETS["export"] = (export_html.parent, export_html.name, "#dc-root aside")
...
if __name__ == "__main__":
    sys.exit(main(Path(sys.argv[1]).resolve(), Path(sys.argv[2])))
