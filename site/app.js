...
// Static rewrite of a Claude Design export, without React or CDNs. The constants block below is the
// export's, kept as is: corrections and new strings go to DICT_FIXES.
"use strict";
(() => {
// ---------- constants (from the Claude Design export) ----------
const PT = 0.352778;
...
// ---------- strings added or corrected since the export ----------
// The block above stays as exported; every change to its texts is listed here.
const DICT_FIXES = {
...
// Breaks at ASCII whitespace only: a no-break space (U+00A0) keeps two words together.
...
function buildInner(it) {
  ...
  // Every line advances 1.2 x its size in put() below, so the estimate uses 1.2 too.
...
function sheetSVG(pageItems, real, clipId) {
  ...
  // smallest size) is cut instead of printing on the next label.
...
// one used to overwrite the results of the last.
let searchSeq = 0;
...
function viewValues() {
  ...
  // decimal separator of the current language: ×1,59 in French
  ...
  return {
    ...
    qr: s.qr,
    // an imported image shows as a thumbnail, a picked icon as its path
    hasIcon: !!act.iD,
