...
Outputs:
  ...
  site/vendor/SOURCES.json        local path -> source URL (as the design export requests it) + SHA-256
  test/site/reference/vendor/     React UMD builds, only needed to run the design export offline in tests
  test/site/reference/SOURCES.json

The design export (design/label-generator.dc.html) loads these from CDNs; the static site and the
fidelity harness load them from here, so nothing is fetched from the network at runtime.
...
DESIGN = ROOT / "design" / "label-generator.dc.html"
...
def label_font_url(family):
    """The exact stylesheet URL built by loadFont() in the design export."""
...
def design_constants():
    """FONTS and MAT arrays, read from the design export so the lists never drift."""
